...
def start_lidar_anim():
    ...
    if lidar_inst is None:
        ...
        if lidar_inst is None: return
    if lidar_ax: lidar_ax.set_title('Escaneo LIDAR', color='cyan', pad=20, fontsize=12); lidar_fig.canvas.draw_idle()
    if lidar_line is None or lidar_ax is None or lidar_fig is None: return
    if lidar_anim and lidar_anim.event_source.is_running(): return
...
def setup_cameras(indices, labels):
    ...
    for i, idx in enumerate(indices):
        ...
        else: labels[i].configure(image=""); labels[i].image = None

def create_lidar_inst():
    global lidar_inst
    if not path.exists(LINUX_DEVICE_PATH): return None
    try:
        lidar = RPLidar(port=LINUX_DEVICE_PATH, baudrate=LIDAR_BAUD_RATE, timeout=LIDAR_TIMEOUT)
        lidar.start_motor(); time.sleep(0.2)
        try: lidar.get_info(); lidar.get_health()[0]
        except RPLidarException as e: pass
        lidar_inst = lidar; return lidar
    except (RPLidarException, OSError) as e: return None

def update_frame_lidar(frame_num):
    global lidar_line, lidar_ax, lidar_inst, lidar_fig
    if lidar_inst is None or lidar_line is None or lidar_ax is None: return ()
    try:
        if not hasattr(lidar_inst, 'iter_scans'): return ()
...
def stop_lidar_anim():
    ...
    if lidar_anim: lidar_anim.event_source.stop(); lidar_anim = None
...
def on_closing(root_window):
    stop_lidar_anim(); stop_all_camera_threads()
    global ser;
